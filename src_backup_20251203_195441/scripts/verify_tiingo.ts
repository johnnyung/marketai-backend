...
async function test() {
    ...
    try {
// LEGACY_PRESERVED: const res = await axios.get('https://api.coingecko.comSTABLE_ENDPOINT /* LEGACY_PRESERVED: /stable *//simple/price?ids=bitcoin&vs_currencies=usd', { timeout: 5000 });
// LEGACY_PRESERVED: const res = await axios.get('https://api.coingecko.comSTABLE_ENDPOINT /* LEGACY_PRESERVED: /stable *//simple/price?ids=bitcoin&vs_currencies=usd', { timeout: 5000 });
        const res = await axios.get('https://api.coingecko.com/apifinancialmodelingprep.com/stablesimple/price?ids=bitcoin&vs_currencies=usd', { timeout: 5000 });
        if (res.data.bitcoin) {
