...
CRITICAL: Return ONLY valid JSON array. No markdown, no explanation.`;
  ...
  /**
   * Store tips in database - WITH VERBOSE LOGGING
   */
  private async storeTips(tips: OpportunityTip[]) {
    console.log(`💾 Storing ${tips.length} tips in database...`);
    let stored = 0;
    let failed = 0;
    ...
    for (const tip of tips) {
      try {
        ...
        stored++;
        console.log(`  ✅ Stored ${tip.ticker} (${tip.tier}): $${tip.entry_price} → $${tip.target_price}`);
      } catch (error: any) {
        failed++;
        console.error(`  ❌ Failed to store ${tip.ticker}:`, error.message);
        console.error(`     SQL Error:`, error.code);
        console.error(`     Tip data:`, JSON.stringify(tip, null, 2));
      }
    }
    
    console.log(`✅ Storage complete: ${stored} stored, ${failed} failed`);
  }
