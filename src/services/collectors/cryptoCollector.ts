...
export async function collectCoinGecko() {
    ...
    try {
        const res = await axios.get('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,solana&vs_currencies=usd&include_24hr_change=true', { timeout: 4000 });
        if (res.data.bitcoin) {
